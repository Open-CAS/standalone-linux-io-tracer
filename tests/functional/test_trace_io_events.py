    ...
    for disk in TestRun.dut.disks:
        ...
        with TestRun.step("Verify trace correctness"):
            ...
            result = any(
                ...
                'lba' in event['io'] and
                int(event['io']['lba']) == int(
                    write_offset / iotrace_lba_len) and
                int(event['io']['len']) == int(
                    write_length.get_value() / iotrace_lba_len) and
            ...
            result = any(
                ...
                'lba' in event['io'] and
                int(event['io']['lba']) == int(
                    read_offset / iotrace_lba_len) and
                int(event['io']['len']) == int(
                    read_length.get_value() / iotrace_lba_len) and
                f"/dev/{event['device']['name']}" == disk.system_path for
                event in events_parsed)
            if not result:
            ...
            result = any(
                ...
                'lba' in event['io'] and
                int(event['io']['lba']) == int(
                    discard_offset / iotrace_lba_len) and
                int(event['io']['len']) == int(
                    discard_length / iotrace_lba_len) and
                f"/dev/{event['device']['name']}" == disk.system_path for
                event in events_parsed)
            if not result:
    ...
    for disk in TestRun.dut.disks:
        ...
        with TestRun.step("Send write commands"):
            for bucket_index in range(number_buckets):
                for i in range(bucket_index + 1):
                    ...
                    seek = (start_lba + bucket_index * bucket_size
                            + randrange(bucket_size)) * iotrace_lba_len \
            ...
            for bucket_index in range(number_buckets):
                for i in range(bucket_index + 1):
                    seek = (start_lba + bucket_index * bucket_size
                           ...
                           / disk.block_size.get_value()
                    dd = (
                        Dd().input(disk.system_path).output("/dev/null")
                            .count(1).block_size(io_len)
                            .iflag('direct,sync').skip
                        (int(seek)))
                    dd.run()
        ...
        with TestRun.step("Verify histogram correctness"):
            ...
            for bucket_index in range(number_buckets):
                ...
                discard = json[0]['histogram'][0]['discard']
                ['range'][bucket_index]
                total = json[0]['histogram'][0]['total']['range'][bucket_index]
                if int(read['begin']) != start_lba + bucket_index * bucket_size:
                    TestRun.fail(
                        f"Invalid read begin range: {read['begin']} "
                        f"for index {bucket_index}")
                if int(read['end']) != start_lba + (bucket_index + 1) \
                        * bucket_size - 1:
                    TestRun.fail(
                        f"Invalid read end range: {read['end']} "
                        f"for index {bucket_index}")
                if int(read['count']) != bucket_index + 1:
                    TestRun.fail(
                        f"Invalid read count: {read['count']} "
                        f"for index {bucket_index}")

                if int(write['begin']) != start_lba + bucket_index \
                        * bucket_size:
                    TestRun.fail(
                        f"Invalid write begin range: {write['begin']} "
                        f"for index {bucket_index}")
                if int(write['end']) != start_lba + (bucket_index + 1) \
                        * bucket_size - 1:
                    TestRun.fail(
                        f"Invalid write end range: {write['end']} "
                        f"for index {bucket_index}")
                if int(write['count']) != bucket_index + 1:
                    TestRun.fail(
                        f"Invalid write count: {write['count']} "
                        f"for index {bucket_index}")

                if int(discard['begin']) != start_lba + bucket_index \
                        * bucket_size:
                    TestRun.fail(
                        f"Invalid discard begin range: {discard['begin']} "
                        f"for index {bucket_index}")
                if int(discard['end']) != start_lba + (bucket_index + 1) \
                        * bucket_size - 1:
                    TestRun.fail(
                        f"Invalid discard end range: {discard['end']} "
                        f"for index {bucket_index}")
                if int(discard['count']) != bucket_index + 1:
                    TestRun.fail(
                        f"Invalid discard count: {discard['count']} "
                        f"for index {bucket_index}")

                if int(total['begin']) != start_lba + bucket_index \
                        * bucket_size:
                    TestRun.fail(
                        f"Invalid total begin range: {total['begin']} "
                        f"for index {bucket_index}")
                if int(total['end']) != start_lba + (bucket_index + 1) \
                        * bucket_size - 1:
                    TestRun.fail(
                        f"Invalid total end range: {total['end']} "
                        f"for index {bucket_index}")
                if int(total['count']) != 3 * (bucket_index + 1):
                    TestRun.fail(
                        f"Invalid total count: {total['count']} "
                        f"for index {bucket_index}")
