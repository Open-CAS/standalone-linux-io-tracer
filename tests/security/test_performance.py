...
def test_data_performance_120s():
    ...
    for disk in TestRun.dut.disks:
        with TestRun.step("Run test random read workload without tracing"):
            results = run_workload(disk.system_path, runtime, verify=False,
                                   num_jobs=jobs, method=method)
            clean_iops = sum(job.read_iops() for job in results)
            TestRun.LOGGER.info(
                "Recorded " + str(clean_iops) + " IOPS without tracing")
        ...
        with TestRun.step("Run test random read workload with tracing"):
            trace = run_workload(disk.system_path, runtime, verify=False,
                                 num_jobs=jobs, method=method)
            trace_iops = sum(job.read_iops() for job in trace)
            TestRun.LOGGER.info(
                "Recorded " + str(trace_iops) + " IOPS with tracing")
